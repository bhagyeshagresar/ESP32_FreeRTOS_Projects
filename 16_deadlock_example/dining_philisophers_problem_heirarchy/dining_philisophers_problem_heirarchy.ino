//Dining Philisophers Problem - Deadlock prevention using Heirarchy(also solves livelock) - Credits Shawyn Hymel
...
void eat(void *parameters) {
  ...
  // Take lower numbered chopstick first
  xSemaphoreTake(chopstick[idx_1], portMAX_DELAY);
  sprintf(buf, "Philosopher %i took chopstick %i", num, idx_1);
  Serial.println(buf);
  ...
  sprintf(buf, "Philosopher %i took right chopstick %i", num, idx_2);
  Serial.println(buf);
  ...
  sprintf(buf, "Philosopher %i returned chopstick %i", num, idx_2);
  Serial.println(buf);

  // Put down lower numbered chopstick
  xSemaphoreGive(chopstick[(num+1)%NUM_TASKS]);
  sprintf(buf, "Philosopher %i returned chopstick %i", num, idx_1);
  Serial.println(buf);
